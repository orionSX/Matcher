...
import { Plus, Search, Users, MapPin, Trophy, MessageSquare, Flame } from 'lucide-react'
...
import SwipeCard from '@/components/mvpblocks/swipe-card'
...
const mockProfiles: MatchProfile[] = [
  ...
  },
  {
    ...
    rank: 'Grandmaster',
    description: 'Main топ, могу на мид и лес. Ищу команду для Flex и Clash. Только с микрофоном.',
    goal: 'Flex',
    age: 23,
    playTime: 'Вечер',
    createdAt: new Date().toISOString(),
  },
  {
    ...
    roles: ['Support'],
    rank: 'Platinum',
    description: 'Саппорт мейн, ищу ADC для дуо. Играю Thresh, Nautilus, Leona.',
    goal: 'Ranked',
    age: 21,
    playTime: 'Любое время',
    createdAt: new Date().toISOString(),
...
export default function Matches() {
  ...
  const [hotProfiles, setHotProfiles] = useState<MatchProfile[]>([])
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (savedProfiles) {
        ...
        const data = parsed.length > 0 ? parsed : mockProfiles
        setProfiles(data)
        setHotProfiles(data.slice(0, 5)) // Первые 5 - горячие
      } else {
        setProfiles(mockProfiles)
        setHotProfiles(mockProfiles.slice(0, 5))
  ...
  const handleSwipe = (direction: 'left' | 'right') => {
    ...
    setTimeout(() => {
      setCurrentHotIndex((prev) => prev + 1)
    }, 300)
  }

  const handleLike = () => {
    console.log('Liked profile:', hotProfiles[currentHotIndex])
    handleSwipe('right')
  }

  const handlePass = () => {
    console.log('Passed profile:', hotProfiles[currentHotIndex])
    handleSwipe('left')
  }

  const currentProfile = hotProfiles[currentHotIndex]
  const hasMoreHotProfiles = currentHotIndex < hotProfiles.length
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-background/90 to-black text-foreground pt-24 pb-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <Tabs defaultValue="hot" className="w-full">
          <TabsList className="grid w-full max-w-md mx-auto grid-cols-2 bg-black/40 border border-rose-900/50">
            <TabsTrigger value="hot" className="data-[state=active]:bg-rose-600">
              <Flame className="mr-2 h-4 w-4" />
              {t('matchesFeed.hotProfiles')}
            </TabsTrigger>
            <TabsTrigger value="all" className="data-[state=active]:bg-rose-600">
              <Users className="mr-2 h-4 w-4" />
              {t('matchesFeed.allProfiles')}
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="hot" className="mt-8">
            <div className="mx-auto max-w-xl">
              <div className="mb-6 text-center">
                <h2 className="text-2xl font-bold text-rose-50">{t('matchesFeed.hotProfiles')}</h2>
                <p className="mt-2 text-sm text-muted-foreground">{t('matchesFeed.hotSubtitle')}</p>
              </div>

              {isLoading ? (
                <Card className="h-[500px] border-rose-900/50 bg-black/40">
                  <CardContent className="flex h-full items-center justify-center">
                    <Skeleton className="h-full w-full" />
                  </CardContent>
                </Card>
              ) : hasMoreHotProfiles && currentProfile ? (
                ...
                <Card className="h-[500px] border-rose-900/50 bg-black/40 backdrop-blur">
                  <CardContent className="flex h-full flex-col items-center justify-center text-center">
                    <Flame className="h-16 w-16 text-rose-400 mb-4" />
                    <p className="text-lg font-medium text-rose-50 mb-2">{t('matchesFeed.noMoreProfiles')}</p>
                    <p className="text-muted-foreground">{t('matchesFeed.checkBackLater')}</p>
                  </CardContent>
                </Card>
              )}
            </div>
          ...
          <TabsContent value="all" className="mt-8">
            <div className="mb-6 text-center">
              <h2 className="text-2xl font-bold text-rose-50">{t('matchesFeed.allProfiles')}</h2>
              <p className="mt-2 text-sm text-muted-foreground">{t('matchesFeed.allSubtitle')}</p>
            </div>
